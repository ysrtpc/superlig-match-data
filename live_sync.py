...
def git_commit_and_push(commit_msg: str):
    try:
        ...
        subprocess.run(["git", "add", FIXTURES_FILE], check=False)
        
        # Check if there are staged changes
        res = subprocess.run(["git", "status", "--porcelain"], capture_output=True, text=True)
        if FIXTURES_FILE not in res.stdout:
        ...
        for attempt in range(3):
            ...
            if push_res.returncode == 0:
                ...
                print(f"[UYARI] Push denemesi {attempt+1} basarisiz, rebase yapiliyor...")
                subprocess.run(["git", "pull", "--rebase"], check=False)
                time.sleep(2)
        print("[HATA] GitHub'a push yapilamadi!")
    except Exception as e:
        print(f"[HATA] Git commit/push hatasi: {e}")

def run_sync_cycle(clock_state: dict) -> tuple[bool, list]:
    ...
    if has_changes:
        ...
        if os.environ.get("GITHUB_ACTIONS"):
            ...
            print(f"[YEREL] fixtures_2026_2027.json kaydedildi: {commit_msg}")
...
def main():
    ...
    while time.time() < end_time:
        try:
            ...
            cur_time = time.strftime("%H:%M:%S")
            if live_list and not has_changes:
    ...
    print("Belirtilen sure tamamlandi. Bot durduruldu.")
